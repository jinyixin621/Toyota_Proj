...
% --------------------------------------------------------
    ...
    cur_dir = '/home/jinyixin/faster_rcnn-master';
    caffe_dir = fullfile(cur_dir, 'external', 'caffe', 'matlab', caffe_version);
    
    if ~exist(caffe_dir, 'dir')
        warning('Specified caffe folder (%s) is not exist, change to default one (%s)', ...
            caffe_dir, fullfile(cur_dir, 'external', 'caffe', 'matlab'));
        caffe_dir = fullfile(cur_dir, 'external', 'caffe', 'matlab');
    end
